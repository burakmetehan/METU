...
* If the destination of second train does not match with the source of the first one, then it produces the string of "Transfer is not possible!" with NO NEW LINE AT THE END.
* Otherwise it combines the routes and calculate the total eta and produces the string in the form of "source1 -> destination1 -> destination2 in # mins."
...
bool operator<(const HighSpeedTrain &h1, const HighSpeedTrain &h2){
    ...
    // sources are equal, comparing the destinations
    comp = h1.to.compare(h2.to);
    if (comp < 0) {
        return true;
    }
    else if (comp > 0)
    ...
    // destionations are equal, comparing the etas
