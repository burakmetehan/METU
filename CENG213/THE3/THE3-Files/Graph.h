...
struct GraphVertex
{
    ...
    GraphVertex(const std::string &vertex_name) {
...
struct GraphEdge
{
    ...
    GraphEdge(int vertex_id_0, int vertex_id_1, int weight) {
        this->weight = weight;
        this->vertexId0 = vertex_id_0;
        this->vertexId1 = vertex_id_1;
        this->masked = false;
    }
};
...
class Graph
{
    private:
        ...
        // Defined Private Helper Functions
        bool duplicate_vertex_check(const std::string &vertex_name) {
            // Return true if there is duplicate otherwise false
            int i, size = vertexList.size();

            for (i = 0; i < size; i++) {
                if (vertexList[i].name == vertex_name) {
                    return true;
                }else;
            }
        ...
        int find_vertex(const std::string &vertex_name) const {
            // Returns index of the vertex if it is in vertexList, otherwise -1 it is not in the vertexList
            int i, size = vertexList.size();
        ...
        int find_connection(int vertex_id_0, int vertex_id_1) {
            // Returns the index of the connection if there is a connection between vertex_id_0 and vertex_id_1. Otherwise, returns -1
            int i, size = edgeList.size();

            for (i = 0; i < size; i++)
            {
                if ((edgeList[i].vertexId0 == vertex_id_0 && edgeList[i].vertexId1 == vertex_id_1) || (edgeList[i].vertexId0 == vertex_id_1 && edgeList[i].vertexId1 == vertex_id_0)) {
                    return i;
                }else;
            }
        ...
        int find_max_weighted_edge(const std::vector<int> &vertex_list) {
            ...
            for (i = 0; i < size-1; i++)
            {
                edge_index = find_connection(vertex_list[i], vertex_list[i+1]);

                if (edge_index == -1)
                    continue;

                if (edgeList[edge_index].weight > max_weight) {
                    max_weight = edgeList[edge_index].weight;
                    max_weight_index = edge_index;
                } else;
            }
