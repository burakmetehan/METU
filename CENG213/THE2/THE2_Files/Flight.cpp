...
bool Flight::addTicket(const Ticket &ticket) {
    int economy_count = 0, business_count = 0, i;
    ...
    if (ticket.getTicketType() == economy && economyCapacity > economy_count)
    {
        tickets.push_back(ticket);
        return true;
    }
    else if (ticket.getTicketType() == business && businessCapacity > business_count)
    {
        tickets.push_back(ticket);
        return true;
    }
...
bool Flight::operator<(const Flight &rhs) const {
    return flightCode < rhs.getFlightCode();
}
